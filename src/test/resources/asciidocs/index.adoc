...
Since the service is primarily intended to play a role in the Deep Dive Curriculum, we reserve the right to change the specifications below _without warning, at any time._ This service should not be consumed by any production-level client, and is made available (as stated in the http://www.apache.org/licenses/LICENSE-2.0[Apache License 2.0], under which the source code underlying the service is published) "on an 'AS IS' BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied."
...
[[license_notices]]
== Credit, copyrights, and licenses
...
(C) 2022 CNM Ingenuity, Inc. All rights reserved.
