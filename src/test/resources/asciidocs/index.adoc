...
The Codebreaker Solitaire service implements the codemaker role as a RESTful web service, with endpoints allowing a client (in the codebreaker role) to start, query, and delete games, and to submit and query guesses.
...
An in-progress (unsolved) game is one in which no guess that matches the secret code exactly has yet been submitted.
