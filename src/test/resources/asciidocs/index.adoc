...
[[post_game_endpoint]]
=== Generate code & start game

==== Request

`POST /codebreaker/games`

Starts a new game by generating a code using the properties specified in the request body.
...
Returns a subset of games (all, unsolved, or solved), in descending order of creation date (i.e. most recently created first).
...
include::{snippets}/games/post-valid/path-parameters.adoc[]
