...
{projectVersion}, {projectTimestamp}
:doctype: book
...
Each guess consists of a string of characters (which must be of the same length as the code---that is, the length specified when starting the game). When a guess is submitted, the response from the codemaker (primarily) consists of 2 numbers:
